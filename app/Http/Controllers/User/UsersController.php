    ...
    public function chat_store(request $request){
        ...
        */
        ...
        if (!empty($file)) {
            ...
            // $request->file->storeAs(self::RESEARCHES_PATH, $fileName);

            unlink(self::RESEARCHES_PATH . $fileName);
            upload($request->file,"admin-assets/uploads/chats_pictures/",$new_file);
            File::copy(
                public_path('admin-assets/uploads/chats_pictures/' . $new_file),
                public_path(self::RESEARCHES_PATH.$new_file)
            );
            DB::table('users_researches')->where('id', $request->research_id)->update([
                'file' => $new_file
            ]);
