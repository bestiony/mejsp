<div id="freelancers">
                    ...
                    <div class="col-lg-2 mt-2 mt-lg-0">
                        ...
                        <div class="modal fade" id="create_list_model" tabindex="-1" role="dialog"
                            ...
                            <div class="modal-dialog" role="document">
                                @livewire('admin.create-email-list')
                            </div>
                        </div>
                    </div>
                    <div class="col-lg-2 mt-2 mt-lg-0">
        ...
        @if (count($lists) == 0)
            <div class="col-12">
                <div class="box-white py-5">
                    <h5 class="mb-0 text-center">لا يوجد قوائم !</h5>
                </div>
            </div>
        @else
                        ...
                        <tbody>
                            @foreach ($lists as $row)
                                <tr>
                                    ...
                                    <td><button
                                        {{-- data-toggle="modal" data-target="#edit_list_model-{{ $row->id }}" --}}
                                            {{-- wire:key="{{ $row->id }}" --}} {{-- wire:click='selectEmailList({{ $row->id }})' --}}
                                            wire:click='updateShow({{$row->id}})'
    ...
    <div>
        @foreach ($lists as $list)
            <div class="modal fade" id="edit_list_model-{{ $list->id }}" tabindex="-1" role="dialog"
                aria-labelledby="edit_list_model" aria-hidden="true">
                <div class="modal-dialog" role="document">
                    @if ($show == $list->id)
