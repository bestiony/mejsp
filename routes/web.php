...
Route::middleware(['UserAuth'])->group(function () {
    Route::prefix(userPrefix())->group(function () {
        Route::prefix('documents')->group(
            function () {
                Route::controller(DocumentController::class)->group(
                    function () {
                        Route::get('', 'user_index')->name('user_documents');
                    }
                );
            }
        );
        ...
        Route::get('logout', [LogoutController::class, 'user_logout']);
        ...
        Route::prefix('conference')->group(function () {
            ...
            Route::controller(CheckoutController::class)->group(function () {
                Route::get('checkout/{id}', 'checkout_conference');
                Route::post('response/checkout', 'checkout_conference_response');
            });
        });
        ...
        Route::prefix('researches')->group(function () {
            ...
            Route::controller(CheckoutController::class)->group(function () {
                Route::get('checkout/{id}', 'checkout_researches');
                Route::post('response/checkout', 'checkout_researches_response');
            });
        });
...
Route::get("journals", [JournalsController::class, 'journals']);
Route::prefix('journal')->group(function () {
    Route::controller(JournalsController::class)->group(function () {
        Route::get('{slug}', 'show');
    });
});
Route::get("versions/{slug}", [JournalsController::class, 'versions']);
Route::get("team/{slug}", [JournalsController::class, 'team']);
Route::get("publication-ethics/{slug}", [JournalsController::class, 'publication_ethics']);
Route::get("how-to-submit-the-article/{slug}", [JournalsController::class, 'how_to_submit_the_article']);
Route::get("publication-price/{slug}", [JournalsController::class, 'publication_price']);
Route::get("reviewers-instructions/{slug}", [JournalsController::class, 'reviewers_instructions']);
Route::get("international-credits/{slug}", [JournalsController::class, 'international_credits']);
Route::get("authors-instructions/{slug}", [JournalsController::class, 'authors_instructions']);
Route::prefix('researches')->group(function () {
    Route::controller(JournalsResearchesController::class)->group(function () {
        Route::get('{slug}/version/{version_id}', 'researches');
        Route::get('{slug}', 'show');
    });
});
Route::get("blog", [ArticlesController::class, 'blog']);
Route::prefix('article')->group(function () {
    Route::controller(ArticlesController::class)->group(function () {
        Route::get('{slug}', 'show');
    });
});
Route::get("en/blog", [ArticlesEnController::class, 'blog']);
Route::prefix('en/article')->group(function () {
    Route::controller(ArticlesEnController::class)->group(function () {
        Route::get('{slug}', 'show');
    });
});
Route::get("blog-single.php", [ArticlesController::class, 'old_article']);
Route::post("subscribe", [SubscribersController::class, 'store']);
Route::get("download-my-certificate/{token}", [UsersController::class, 'download_certificate']);
...
Route::prefix('invoice')->group(function () {
    ...
    Route::controller(CheckoutController::class)->group(function () {
        Route::get('response/checkout', 'checkout_invoice_response');
    });
